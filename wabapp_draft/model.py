...
class TransformerLinearModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_dict, max_length,
                 encoder_heads, encoder_layers, decoder_heads, decoder_layers, pe_scaling):
        super(TransformerLinearModel, self).__init__()
        
        self.encoder = TransformerEncoderLinear(input_size, hidden_size, encoder_heads, encoder_layers, pe_scaling)
        self.decoder = TransformerDecoder(hidden_size, output_dict, max_length, decoder_heads, decoder_layers, pe_scaling)
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000, scaling=False):
...
class TransformerEncoderConv2d(nn.Module):
    def __init__(self, input_size, hidden_size, encoder_heads, encoder_layers, pe_scaling):
        ...
        self.position_encoding = PositionalEncoding(self.d_model, scaling=pe_scaling)
...
class TransformerEncoderLinear(nn.Module):
    def __init__(self, input_size, hidden_size, encoder_heads, encoder_layers, pe_scaling):
        ...
        # self.input_proj = nn.LSTM(input_size, hidden_size, num_layers=1, batch_first=True)
        self.position_encoding = PositionalEncoding(self.d_model, scaling=pe_scaling)
        # self.dropout = nn.Dropout(p=0.3)
        self.encoder_layer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.hidden_size, nhead=self.nhead, batch_first=True, dropout=0.3),
            num_layers=self.num_layers)
...
class TransformerDecoder(nn.Module):
    def __init__(self, hidden_size, output_dict, max_length, decoder_heads, decoder_layers, pe_scaling):
        ...
        # self.input_proj = nn.LSTM(self.d_model, hidden_size, num_layers=1, batch_first=True)
        self.position_encoding = PositionalEncoding(self.d_model, max_length, pe_scaling)
        # self.dropout = nn.Dropout(p=0.3)
        self.decoder_layer = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=self.hidden_size, nhead=self.nhead, batch_first=True, dropout=0.3),
            num_layers=self.num_layers)
    ...
    def forward(self, encoder_output, tgt=None, max_loop=None, tgt_mask=None):
        ...
        if tgt is not None:
            ...
            for _ in range(max_loop):
                ...
                # tokens, _ = self.input_proj(tokens)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_hidden, target_onehot=None, max_loop=None):
        ...
        if target_onehot is not None:
            ...
            decoder_input[:, :, self.all_note.index("<SOS>")] = 1
        ...
        for i in range(1, max_loop+1):
            ...
            for b in range(decoder_input.size(0)):
                ...
                if topi == self.all_note.index("<EOS>") and target_onehot is None:
                    # print("eos")
                    break
                else:
