...
from inference import Inference_Chunk, Inference_Unchunk
...
def main_chunk():
    ...
    log_tags = [model_name, audio_dir, 'chunk-train', 'double-time-loss', "midi-label"]
    log_notes = ""
    random_state = 0
    experiment_name = model_name + "_chunk_midilabel_" + str(random_state)
    ...
    train_set = MusicDataset_Chunk(annotations_file = train_metadata,
               audio_dir = audio_dir,
    ...
    'transformer_linear': (TransformerLinearModel, {'input_size':input_size, 'hidden_size' : 256, 'output_dict':output_dict, 'max_length':max_length,
                                                     ...
                                                     'decoder_heads' : 4, 'decoder_layers' : 4, 'pe_scaling':False}),
    ...
    config={
        ...
        'time_length':time_length, 'chunk_size':chunk_size, 'max_length': max_length, 
    ...
    model_optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=0.0001)
    ...
    if (force_start == "YES" ):
        ...
        print(Inference_Chunk(model, config, LOG_PATH, addition=None, wandb=wandb))
...
def main_unchunk():
    ...
    log_notes = None
    random_state = 0
    experiment_name = model_name + "_unchunk_midilabel_" + str(random_state)
    ...
    train_set = MusicDataset_Unchunk(train_metadata,
                    audio_dir,
                    MEL_SPECTROGRAM,
                    SAMPLE_RATE,
                    time_length,
    ...
    'transformer_linear': (TransformerLinearModel, {'input_size':input_size, 'hidden_size' : 256, 'output_dict':output_dict, 'max_length':max_length,
                                                     'encoder_heads' : 4, 'encoder_layers' : 4,
                                                     'decoder_heads' : 4, 'decoder_layers' : 4, 'pe_scaling':True}),
    'transformer_conv2d': (TransformerConv2dModel, {'input_size':input_size, 'hidden_size' : 256, 'output_dict':output_dict, 'max_length':max_length,
                                                     'encoder_heads' : 4, 'encoder_layers' : 4,
                                                     'decoder_heads' : 4, 'decoder_layers' : 4, 'pe_scaling':True})
    }
    ...
    if (force_start == "YES" ):
        ...
        run_training(model, train_set, validation_set, config, wandb)
        print(Inference_Unchunk(model, config, LOG_PATH, addition=None, wandb=wandb))
        print(InferencePlot_Unchunk(LOG_PATH, output_dict, wandb))
