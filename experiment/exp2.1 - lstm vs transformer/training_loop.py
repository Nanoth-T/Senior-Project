    ...
    for epoch in range(1, num_epochs+1):
        ...
        for batch in batch_train:
            ...
            for each in range(decoder_outputs.size(0)):
                ...
                acc_all = sequence_accuracy(predicted, expected)
        ...
        print(f"{timeSince(start)} (Epoch {epoch}/{num_epochs}), Loss: {np.average(total_loss):.4}, Accuracy All: {np.average(total_acc_all):.4}")
        ...
        with torch.no_grad():
            ...
            for batch in batch_val:
                ...
                # print(event_output_vals, pitch_output_vals, time_output_vals)
                ...
                for each_val in range(decoder_output_vals.size(0)):
                    ...
                    acc_all_val = sequence_accuracy(predicted_val, expected_val)
            ...
            print(f"Validate Set, Loss: {np.average(total_loss_val):.4}, Accuracy All: {np.average(total_acc_all_val):.4}")
