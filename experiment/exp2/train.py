...
def train(ende_model, num_epochs, train_set, criterion, 
    ...
    for epoch in range(num_epochs):
        ...
        batch_train = create_data_loader(train_set_shuffle, batch_size)
        batch_test = create_data_loader(test_set_shuffle, batch_size)
        ...
        for batch in batch_train:
            ...
            decoder_outputs = ende_model(signal_tensor, target_onehot=target_onehot)
        ...
        with torch.no_grad():
            ...
            for batch_val in batch_test:
                ...
                decoder_outputs_val = ende_model(signal_val, max_loop=max_length)
                # print(target_val)
            ...
            if sum(total_loss_val) < best_validation_loss:
                ...
                if experiment is None:
                    ...
                    torch.save(ende_model.state_dict(), f"{str(current_path)}/exp_log/{str(experiment)}/ende_model.pth")
...
if __name__ == "__main__":
    ...
    experiment = f"{AUDIO_DIR}_timelength200_3rd"
    # experiment = None
    if experiment is None:
    ...
    # writer.add_graph(decoder, train_set[0])
    ...
    if str(input("YES|NO : ")) == "YES": 
        ...
        if experiment is None:
            ...
            torch.save(ende_model.state_dict(), f"{str(current_path)}/exp_log/{str(experiment)}/last_ende_model.pth")
        print("Model trained and stored at model.pth")

        path = f"{str(current_path)}/exp_log/{str(experiment)}"
        ...
        shutil.rmtree(f'{str(current_path)}/exp_log/{str(experiment)}')
