...
def predict(encoder, decoder, signal, max_length):
    ...
    with torch.no_grad():
        ...
        decoder_outputs, _ = decoder(encoder_hidden, max_loop=max_length)
...
if __name__ == "__main__":
    ...
    experiment = f"exp_log/{AUDIO_DIR}3rd_timelength200"
    # experiment = None
    ...
    print(f"All Accuracy Exactly: {np.average(all_acc_exactly):.2%}")
    print(f"All Accuracy Length: {np.average(all_acc_len):.2%}")
    print(f"All Accuracy is in: {np.average(all_acc_in):.2%}")
    print(f"All Accuracy Average: {np.average(all_acc_avg):.2%}")
