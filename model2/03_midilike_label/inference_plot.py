...
def plot_wrong_norm(wrong_dict):
    fig, ax = plt.subplots(1, 1, figsize=(8,10))
    plt.bar_label(plt.bar(wrong_dict.keys(), [round(wrong_dict[i]["count"] / len(wrong_dict[i]["true"]), 4) for i in wrong_dict.keys()]), fontsize=16)
...
if __name__ == "__main__":
    ...
    LOG_PATH = "08_midilike/log/dataset/midilike_diff5note_data/transformer_linear_dataset/midilike_diff5note_data_020240909-0154"
    output_dict = None

    wandb.init(entity='rhythm-tempo-project', project='senior-project', id='opvenpni', resume='must')
