...
def collate_fn(batch):
    ...
    for signal, input_ten, label_ten in batch:
        signal_list.append(signal)
        input_list.append(input_ten)
        labeltensor_list.append(label_ten)

    # Pad the sequences in each list
    signal_batch = pad_sequence(signal_list, padding_value=0, batch_first=False) # เดิม batch_first=True
    onehot_batch = pad_sequence(input_list, padding_value=0, batch_first=False) # เดิม batch_first=True
    labeltensor_batch = pad_sequence(labeltensor_list, padding_value=-1, batch_first=False) # เดิม batch_first=True
...
if __name__ == "__main__":
    ...
    for batch in train_data_loader:
        ...
        print("Signals Shape:", signals.shape, signals)
        print("Onehots Shape:", target_onehots.shape, target_onehots)
