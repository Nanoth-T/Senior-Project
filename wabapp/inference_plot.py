...
def InferencePlot_Chunk(LOG_PATH, output_dict, wandb):
    ...
    cfm_all = plot_confusion_matrices(wrong_dict, wandb=wandb)
    wandb.log({'Confusion Matrix/All': wandb.Image(cfm_all)})


    file = "test_data_FULL.csv"
    file_dir = f"{LOG_PATH}/{file}"
    tags = "FullAudio."

    data = pd.read_csv(file_dir)

    wandb.log({'FullAudio.Acc/Excatly Accuracy Sequence': wandb.Image(acc_sequence(data["decode_predict"], data["decode_target"]))})
    wandb.log({'FullAudio.Acc/Correct Length Accuracy': wandb.Image(correct_length(data["decode_predict"], data["decode_target"]))})

    wrong_index_ls, wrong_dict = wrong_index(data["decode_predict"], data["decode_target"])
    wandb.log({'FullAudio.Incorrect/Incorrect idx': wandb.Image(wrong_index_ls)})
    wandb.log({'FullAudio.Incorrect/Incorrect idx - norm': wandb.Image(plot_wrong_norm(wrong_dict))})

    cfm_all = plot_confusion_matrices(wrong_dict, tags, wandb=wandb)
    wandb.log({'FullAudio.Confusion Matrix/All': wandb.Image(cfm_all)})
    
    return "log completed"


def InferencePlot_Unchunk(LOG_PATH, output_dict, wandb):
...
def plot_confusion_matrices(wrong, tags="", wandb=None):
    amount_in_one_pic = 6
    ...
    for index, data in wrong.items():  # Iterate over dictionary entries
        ...
        if index%amount_in_one_pic == 5 or index == len(wrong)-1:
            ...
            if wandb is not None:
                if tags:
                    wandb.log({f'{tags}Confusion Matrix/{index//amount_in_one_pic}) Index{index-(row*2 + col)}-{index}': wandb.Image(fig)})
                else:
                    wandb.log({f'{tags}Confusion Matrix/{index//amount_in_one_pic}) Index{index-(row*2 + col)}-{index}': wandb.Image(fig)})
...
if __name__ == "__main__":
    ...
    print(InferencePlot_Chunk(LOG_PATH, output_dict, wandb))
