...
def main():
    ...
    train_metadata = "dataset/long_length_data/train_metadata_chunks_fixed_time.csv"
    test_metadata = "dataset/long_length_data/test_metadata_chunks_fixed_time.csv"
    validation_dir = None
    model_name = "transformer_linear" # ['lstm', 'transformer_linear', 'transformer_conv2d']
    log_tags = [model_name, audio_dir, "midi-like", "chunks-train", "custom-loss"]
    log_notes = ""
    random_state = 0
    ...
    # Initialize WandB
    wandb.init(project="chunks_train", config=config,
    ...
    range_indices = {'note_on': list(range(0, 128)), 'note_off': list(range(128, 256)) , 'time_shift': list(range(256, 356)), 'velocity': list(range(356, 388))}
