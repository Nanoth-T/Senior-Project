...
def get_tempo(path):
    ...
    tempo = find_tempo_from_wav.predict(path)
    return tempo
...
if __name__ == "__main__":
    ...
    addition = {
        ...
        'file_name' : "audio3_data.csv",
    }
    
    CURRENT_PATH = pathlib.Path(__file__).parent.resolve()

    LOG_PATH = f"09_chunks/log/dataset/long_length_data/transformer_linear_dataset/long_length_data_020241001-2304"

    state_dict = torch.load(f"{LOG_PATH}/model.pth")
    config = state_dict["config_dict"]
    model = None

    # wandb.init(entity='rhythm-tempo-project', project='chunks_train', id='80fn70p2', resume='must')

    print(Inference(model, config, LOG_PATH, addition, wandb=None))
    output_dict = None
