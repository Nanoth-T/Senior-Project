...
def train(encoder, decoder, num_epochs, sound, criterion, 
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            confmat = ConfusionMatrix(task="multiclass", num_classes=len(rhythm_dict), ignore_index=-1)
            total_loss_val = []
            total_acc_val = []
            ...
            for b in validate_set:
                ...
                cf_matrix = confmat(decoder_outputs_val.view(-1, 7), target_val.view(-1)).numpy()
...
if __name__ == "__main__":
    ...
    train_data_loader = create_data_loader(train_sound, batch_size, len(augment)+1)
